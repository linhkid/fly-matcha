...
.PHONY: venv web check test web-test data-guard mechanics-gate census dev
...
check: test web-test data-guard mechanics-gate
...
mechanics-gate:  ## no mechanic is switched on without a passed experiment that gates it (spec rule 1)
	@$(PY) -m flylab exp gate
