...
### The living view, 2026-09-21. The user's notes after watching V3.

#### N18. "The 'his lips, magnified' neurons are not lit", the cloud further away, no dotted line, and more to say about his books
**The user's decisions, 2026-09-21.**
- **His lips.** The dots did flash, and the user could not see it: a flash lasted a third of a second, the touch is short, a low dose fires each neuron once in seconds, and the first bowl of every visit is hot water with nothing in it, which drives nobody. A dot now flashes at the spike and fades over a second and a half. The flash is the spike; the afterglow is ours and the panel says so. Under the dots a line says what this bowl drives, or that it drives nothing, so a dark panel explains itself. The rotation was left alone: it is documented by bowl number in every evidence note. Nothing was lit that did not fire.
- **The cloud** stands further back and a little higher, clear of the card at every window size tried, and **the dotted line from his head to it is gone**. What ties the cloud to him now is the words: "his brain", "his nerve cord", and the card.
- **His books.** Twelve titles, not six, and eight lines for each, not one: 96 lines, written by three writers from different angles and picked by an editor (a small workflow), then checked here by rule: no line quotes a book, none says anything about tea as a taste, none has him worn out, hungry or remembering, none holds a bare number. Which book and which line depend on the bowl and on how many breaks he has been sent on, so two breaks running never give the same book, and every line of a book is heard before one repeats. The count starts somewhere new on each visit (the page's own randomness, in `main.ts` only; a still takes `&visit=` from its address and is repeatable). `reaction.ts` stays free of chance, and a test holds every line to the rules.

#### N19. "Maybe hide the STAGED, just keep it purple"
**The user's decision, 2026-09-21.** On the panels the chip that says STAGED is no longer shown; staged numbers and his words stay purple, and a key at the foot of the page says what the three colours mean. The tag is still data: it is in the markup for a screen reader, in every value's tooltip, and in full in "What is staged here?". The chips for "from the fly" and "our model" stay.

#### N20. "This HUD is a bit big"
**The user's decision, 2026-09-21,** with a screenshot in which the panels filled the lower half of the window and the scene had been scrolled away from them. The words beside the scene are now small on purpose: one size for every panel (`--hud`, twelve and a half pixels), tighter padding, smaller chips and buttons. The card is a box of at most 820 pixels beside the shelf, not a slab across the window, and never taller than two fifths of it. His lips' panel keeps the dots, the two counts and what this bowl drives; what the rings are and what a flash means fold away under "What the dots show". The card's footnote says the same in fewer words: "His words and his movements are puppetry: we move his legs. His motor neurons firing moves nothing here. …". Nothing was removed from the page, only folded or shrunk, and every number still goes through the registry.

#### N21. The card docks at the right
**The user's decision, 2026-09-21:** "make it right align for the action HUD". The card now sits at the bottom right, under his nervous system, which has stood further back and higher since N18: shots at 1440 × 900, 1460 × 760 and 1900 × 1000 with all six legs moving show the card clear of his nerve cord and of him. The shelf with his lips stays at the bottom left.
